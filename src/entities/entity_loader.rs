...
pub fn load_static_data(
    rift_rumble_entity_collection: &mut RiftRumbleEntityCollection,
) -> Result<(), io::Error> {
    //Champions
    ...
    for path in paths {
        ...
        if let true = path.is_file() {
            load_champion_from_file(
                &mut rift_rumble_entity_collection.champions,
    ...
    load_items_from_file(&mut rift_rumble_entity_collection.items, paths.as_str())
        .ok_or(io::ErrorKind::InvalidData)?;

    //Runes
    let paths = DATA_DRAGON_ROOT.to_owned() + "/runesReforged.json";
    load_runes_from_file(&mut rift_rumble_entity_collection.runes, paths.as_str())
        .ok_or(io::ErrorKind::InvalidData)?;

    //Summoners
    let paths = DATA_DRAGON_ROOT.to_owned() + "/summoner.json";
    load_summoners_from_file(&mut rift_rumble_entity_collection.summoners, paths.as_str())
        .ok_or(io::ErrorKind::InvalidData)?;

    Ok(())
}

fn load_champion_from_file(
    champions: &mut Vec<Arc<RiftRumbleEntity>>,
    file_path: &str,
) -> Option<()> {
    ...
    champions.push(Arc::new(RiftRumbleEntity::Champion(GenericStaticData {
        id,
        name,
    })));
...
fn load_runes_from_file(
    runes: &mut HashMap<String, HashMap<u8, Vec<Arc<RiftRumbleEntity>>>>,
    file_path: &str,
) -> Option<()> {
    let file = fs::File::open(file_path).ok()?;
    let json: Value = serde_json::from_reader(file).ok()?;
    for entry in json.as_array()? {
        ...
        for (slot_number, content) in object.get("slots")?.as_array()?.iter().enumerate() {
            for rune in content.get("runes")?.as_array()? {
                ...
                slot_row.push(Arc::new(RiftRumbleEntity::Rune(GenericStaticData {
                    id,
                    name,
                })));
            }
...
fn load_items_from_file(items: &mut Vec<Arc<RiftRumbleEntity>>, file_path: &str) -> Option<()> {
    let file = fs::File::open(file_path).ok()?;
    let json: Value = serde_json::from_reader(file).ok()?;
    let json = json.get("data")?.as_object()?;
...
fn load_summoners_from_file(
    ...
    for id in json.keys() {
            ...
            .contains(&Value::String(String::from(CLASSIC_MODE)))
        {
            ...
            summoners.push(Arc::new(RiftRumbleEntity::Summoner(GenericStaticData {
                id: id.to_owned(),
                name,
            })));
        }
